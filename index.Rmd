...
* Question: Are P and U prime charts always the best choice for count data (even with small denominators)? If so, I prime charts may be used for all types of count and measurement data.
...
```{r rxbarchart1, fig.cap='Xbar chart from 24 subgroups of 10 to 20 random normal data. I prime limits a bit too wide.'}
set.seed(1)
...
```{r rxbarchart2, fig.cap='Xbar chart from 24 subgroups of 10 to 20 random normal data. I prime limits close to original limits.'}
set.seed(6)
x <- rep(1:24, round(runif(24, 10, 20)))
y <- rnorm(length(x), 9)
compplot(x, y, chart = 'xbar', title = NULL)
...
```{r rxbarchart3, fig.cap='Xbar chart from 24 subgroups of 10 to 20 random normal data. I prime limits a bit too narrow'}
set.seed(8)
x <- rep(1:24, round(runif(24, 10, 20)))
y <- rnorm(length(x), 9)
compplot(x, y, chart = 'xbar', title = NULL)
