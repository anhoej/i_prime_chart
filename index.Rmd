...
a. How does the I'-chart compare with the I-chartfor subgroup sizes = 1?

a. How does the I'-chart accommodate varying subgroup sizes?

a. How does the I'-chart compare with other widely used SPC charts (P, C, U, and Xbar charts)?
...
Figure \@ref(fig:pchart) is a P control chart of the percentage of patients with bacteremia from Table \@ref(tab:tabbac) who died within 30 days after infection. As seen, the I'-limits are (in this case) a bit wider. However, the difference is, for practical purposes, negligible.

```{r pchart, fig.cap="P chart of proportion patients who died of bacteremia. Grey background: control limits from original chart. Red lines: control limits from I'-chart"}
...
This observation aligns with our experience testing various datasets comparing Shewhart control charts (Xbar, P, C, and U) with I' limits (see Appendix for examples). While the I' limits are sometimes slightly wider or narrower than the original limits, the differences are rarely significant enough to affect the analysis conclusions.
...
```{r ppchart, fig.cap="P'-chart of proportion patients who died of bacteremia."}
compplot(bac$month, bac$deaths, bac$patients,
         chart = 'pp',
         title = NULL,
         xlab = 'Month')
...
As seen (and expected from theory), the I' limits match the P' limits. This observation has also been confirmed with other datasets and with the U- and U'-chart of infection rates as demonstrated below (figures \@ref(fig:uchart) and \@ref(fig:upchart)).
...
```{r upchart, fig.cap="U'-chart of infection rates"}
compplot(bac$month, bac$ha_infections, bac$risk_days,
         chart = 'up',
         multiply = 10000,
         title = NULL,
...
```{r ripchart, fig.cap="I'-chart from 24 random, normal data with denominator."}
compplot(x, y * n, n, chart = 'i', title = NULL)
```

### Xbar charts from multiple random normal measurements

```{r rxbarchart1, fig.cap="Xbar chart from 24 subgroups of 10 to 20 random normal data. I' limits a bit wider than Xbar limits."}
set.seed(1)
