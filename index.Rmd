...
To construct the control charts, we used R version 4.5.0 [@r-base] with qicharts2 version 0.8.0  [@qicharts2], which contains functions for Shewhart control charts in addition to functions for prime charts for proportions and rates with large denominators and for the I'-chart.
...
This observation aligns with our experience testing various datasets comparing Shewhart control charts (Xbar, P, C, and U) with I'-chart limits. While the I'-chart limits are sometimes slightly wider or narrower than the original limits, the differences are rarely significant enough to affect the analysis conclusions.
...
As seen (and expected from theory), the I'-chart limits match the P'-chart limits. This observation has also been confirmed with other datasets and with the U'-chart of infection rates as demonstrated below (figures \@ref(fig:uchart) and \@ref(fig:upchart)).
...
a. For count data, the I'-chart closely approximates the behaviour of Laney’s prime charts (P'-chart and U'-charts).  Also, the I' chart can be used to normalize non-count data, like averages, where P'- and U'-charts do not apply.
...
Our findings suggest that the I'-chart is more sensitive and appropriate in real-world healthcare settings where case volume, exposure time, or other denominators often vary over time.  A key strength of the I'-chart is that it maintains the simplicity of the I-chart while offering a more context-sensitive representation of variation.  

Consequently, the I'-chart has the potential to replace the myriad of SPC charts (Xbar-, I-, C-, U-, P-, U'-, and P'-charts), as it gives consistent results.
