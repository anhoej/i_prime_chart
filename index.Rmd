...
To address this limitation, Taylor recently proposed the normalised I-chart or the I-prime (I'-chart), a modified version of the I-chart, that accommodates varying denominators (or subgroup sizes) [@taylor2018].
...
To demonstrate the use of I prime charts on aggregated measurement data with varying subgroup sizes we used data from a regional outpatient clinic for children with diabetes. For privacy purposes, data were aggregated in advance including the monthly number of children who visited the clinic and their average glycated haemoglobin (HbA1c, a measure of long term blood glucose levels). Date are presented in Table \@ref(tab:tabhba1c).

To construct the control charts, we used R [@r-base] with qicharts2  [@qicharts2], which contains functions for Shewhart control charts in addition to functions for prime charts for proportions and rates with large denominators and for the I prime chart.
...
```{r pchart, fig.cap='P chart of proportion patients who died of bacteremia. Grey background: control limits from original chart. Red lines: control limits from I prime chart'}
...
Figure \@ref(fig:ppchart) plots the same data as Figure \@ref(fig:pchart) but the original control limits have been calculated using the procedure for P prime charts as suggested by Laney [@laney2002; @mohammed2013].
...
1.  The I'-chart produces results that are an exact match to the original I-chart when applied to individual measurement data (subgroup size = 1). This validates the I'-chart as a generalization rather than a replacement, ensuring continuity for users familiar with the original method.

1. The I'-chart closely approximates the behaviour of Laney’s prime charts (P'-chart and U'-charts), because they too consider within and between subgroup variation, and are recommended for analysing proportions and rates, especially when denominators are large.

1. Additionally, while not an exact substitute, the I'-chart shows reasonable alignment with other popular SPC charts (X-bar, C-, U-, and P-charts).
